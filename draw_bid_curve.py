...
def draw_bid_curves_for_multiple_days(data_multiple_days):
    ...
    for i, day in enumerate(unique_days):
        ...
        day_df['SCH_BID_TIMEINTERVALSTART'] = pd.to_datetime(day_df['SCH_BID_TIMEINTERVALSTART']).dt.hour
        day_df['SCH_BID_TIMEINTERVALSTOP'] = pd.to_datetime(day_df['SCH_BID_TIMEINTERVALSTOP']).dt.hour
        
        day_df.sort_values(by=['SCH_BID_TIMEINTERVALSTART', 'SCH_BID_TIMEINTERVALSTOP', 'SCH_BID_XAXISDATA'], inplace=True)
        
        # create the stepwise curve for each day
        unique_pairs = set(zip(day_df['SCH_BID_TIMEINTERVALSTART'], day_df['SCH_BID_TIMEINTERVALSTOP']))
        ...
        for time_start, time_end in unique_pairs_list:
            ...
            for hour in range(time_start, time_end):
                ...
                fig.add_trace(go.Scatter3d(x=x_data, y=y_data, z=z_data, mode='lines', line=dict(color=color), name=f"Date {day} Hour {hour}"))
...
def main():
    ...
    COLUMNS_TO_EXTRACT = param['COLUMNS_TO_EXTRACT'] #!!! do not extract self-schedule columns for now
